...
def get_box_name(available_names, created_instances):
    ...
    running_instances = r.smembers('running_instances')
    stopped_instances = r.smembers('stopped_instances')
    if nval is not None and \
            nval not in running_instances and \
            nval not in stopped_instances:
        boxn = nval.decode('utf-8')
    else:
        if not r.exists(created_instances):
            r.set(created_instances, 0)
